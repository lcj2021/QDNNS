...
partition_hnns_full_feat(const std::vector<data_t>& queries_vectors, std::vector<data_t>& part1, std::vector<data_t>& part2, 
    ...
    assert (0 <= percentage && percentage <= 100 && queries_vectors.size() % dim == 0);
    size_t n = queries_vectors.size() / dim;
    size_t dim_feat = test_full_feat.size() / n;
    std::cout << "[Partition][HNNS] dim_feat: " << dim_feat << std::endl;
    ids1.clear();   part1.clear();
    ids2.clear();   part2.clear();
    if (percentage == 0) {
        part2 = queries_vectors;
        ...
        std::vector<double> scores(n);
        std::string params = "num_threads=" + std::to_string(num_thread);
        LGBM_BoosterPredictForMat(handle, test_full_feat.data(), C_API_DTYPE_FLOAT32, 
            n, dim_feat, 1, C_API_PREDICT_NORMAL, 0, -1, params.data(), &out_len, scores.data());
...
int main(int argc, char** argv) 
{
    ...
    std::vector<data_t> test_full_feats;
    ...
    std::string test_full_feats_path = feat_prefix + dataset + 
    ...
    for (auto pct : pcts) {
        for (int iter = 0; iter < 3; ++iter) {
            ...
            if (method == "random") {
                ...
                knn_all.clear();         dist_all.clear();
                e2e_timer.Reset();    e2e_timer.Start();
                std::tie(nq_cpu, nq_gpu) = partition_hnns_full_feat(queries_vectors, test_vector_cpu, test_vector_gpu, test_ids_cpu, test_ids_gpu, d0, test_full_feats, pct);
                knn_gpu.resize(nq_gpu * k_gpu);
                dist_gpu.resize(nq_gpu * k_gpu);
